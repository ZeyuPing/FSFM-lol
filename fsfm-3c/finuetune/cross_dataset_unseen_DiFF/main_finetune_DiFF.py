...
# --------------------------------------------------------
# V1: modified from /fsfm-3c/finuetune/cross_dataset_DfD/main_finetune_DfD.py
...
def get_args_parser():
    parser = argparse.ArgumentParser('FSFM-3C fine-tuning for deepfake detection '
                                     '(this version is tailed for unseen DiFF benchmarl evaluation)', add_help=False)
    ...
    parser.add_argument('--output_dir', default='',
                        help='path where to save, empty for default: ./checkpoint/{user}/experiments_finetune/{PID}')
...
def get_shared_folder() -> Path:
    ...
    if Path("checkpoint/").is_dir():
        p = Path(f"{cur_file_path}/{user}/experiments_finetune")
        if args.eval:
            p = Path(f"{cur_file_path}/{user}/experiments_finetune/test/")
