...
# --------------------------------------------------------
# V1: modified from /fsfm-3c/finuetune/cross_dataset_DfD/main_test_DfD.py
...
from engine_finetune import train_one_epoch, evaluate, test
...
def main(args):
    ...
    frame_metric = []  # reused for img-level

    model_dir = args.resume

    if args.eval:
        for ds_name, ds_path in cross_dataset_test_path.items():
            args.resume = os.path.join(model_dir, 'checkpoint-min_val_loss_' + ds_name + '.pth')
            ...
            print(f"\n\n--Metrics of the network testing on the {ds_name} dataset with {len(dataset_val)} test imgs--")
            test_stats = test(data_loader_val, model, device)
            frame_metric.extend([
                test_stats['frame_auc'],
                test_stats['frame_eer']])
            del dataset_val, sampler_val, data_loader_val
